...
Parameter& Unigram_Model::get_parameter(int which) {
	switch (which)
	{
		case Unigram_Model::ALPHA:
			return _alpha;
		case Unigram_Model::BETA:
			return _beta;
		case Unigram_Model::RHO:
			return _rho;
		default:
...
mat& Unigram_Model::get_mat(int which)
{
	switch (which)
	{
		case Unigram_Model::MU_0:
			return _mu_0;
		case Unigram_Model::WISHART:
			return _wishart;
		case Unigram_Model::MU:
			return _mu;
		case Unigram_Model::COV:
			return _cov;
		default:
        	LOG(FATAL) << "Unknown parameter being asked from Unigram Model";								
	}
...
void Unigram_Model::set_parameter(int which, mat& param) {
	switch (which)
	{
		case Unigram_Model::MU_0:
			_mu_0 = param;
...
bool Unigram_Model::save() {
    ...
    string mu_dump = context.get_string("mudump");
    string cov_dump = context.get_string("covdump");
