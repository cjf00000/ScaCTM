...
#include "Filter_Accumulator.h"
#include "Filter_EtaSampler.h"
#include "Context.h"
...
void TBB_Pipeline::init() {
    ...
    _etasampler = new Filter_EtaSampler(_refiner);
    _accumulator = new Filter_Accumulator(_refiner);
...
void TBB_Pipeline::add_etasampler()
{
    _pipeline->add_filter(*_etasampler);
}

void TBB_Pipeline::add_accumulator() {
    _pipeline->add_filter(*_accumulator);
}

void TBB_Pipeline::add_gausssampler()
{
	_ifSampleGauss = true;
}
...
void TBB_Pipeline::dump(int lag)
{
    LOG(FATAL) << "Unsupported method TBB_Pipeline::dump";
}
