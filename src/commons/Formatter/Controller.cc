...
int main(int argc, char *argv[]) {
    ...
    LOG(WARNING)
            << "----------------------------------------------------------------------";
    LOG(WARNING) << "Log files are being stored at " << FLAGS_log_dir
            << "/formatter.*";
    LOG(WARNING)
            << "----------------------------------------------------------------------";
    ...
    LOG(INFO) << flagsInp;
    string input_corpus;
    if (google::GetCommandLineFlagInfoOrDie("corpusprefix").is_default) {
        LOG(WARNING)
                << "Assuming that corpus is being piped through stdin. Reading from stdin...";
    }
    else
    {
	input_corpus = FLAGS_corpusprefix + dotrank;
        LOG(WARNING) << "Reading from " << FLAGS_corpusprefix;
    }

    Context& context = Context::get_instance();
    context.put_string("corpus", input_corpus);

    LOG_IF(WARNING, !google::GetCommandLineFlagInfoOrDie("dumpfile").is_default)
	    <<"Will use the dictionary dump " << FLAGS_dumpfile << " to load the global dictionary.";

    string output_prefix = context.get_string("outputprefix");
    string output_w = output_prefix + ".wor" + dotrank;
    ...
    << endl;
    string dict_dump = output_prefix + ".dict" + ".dump" + dotrank;
    if(google::GetCommandLineFlagInfoOrDie("dumpfile").is_default)
